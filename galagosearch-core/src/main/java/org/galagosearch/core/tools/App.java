...
import org.galagosearch.exercises.Tokenizer;
...
public class App {
    ...
    private void handleTestTokenizer(String[] args) throws Exception {
    	TagTokenizer tagTokenizer = new TagTokenizer();
    	Tokenizer tokenizer = new Tokenizer();
    	Document result1 = tagTokenizer.tokenize(args[0]);
    	Document result2 = tokenizer.tokenize(args[0]);
    	System.out.println();
    	System.out.println("*** Test sentence ***");
    	System.out.println(args[0]);
    	System.out.println();
    	System.out.println();
    	System.out.println("*** TagTokenizer result: ");
    	for (String term : result1.terms) {
    ...
    public void run(String[] args) throws Exception {
        ...
        if (command.equals("help") && args.length > 1) {
            ...
            handleSearch(args);
        } else if (command.equals("test-tokenizer")) {
        	handleTestTokenizer(Utility.subarray(args, 1));
        } else if (command.equals("eval")) {
            handleEval(args);
        } else {
            usage();
